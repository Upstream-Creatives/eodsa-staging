...
export async function POST(request: Request) {
  try {
    ...
    if (clients && clients.length > 0) {
      ...
      // Check if they have announcer dashboard access
      if (!allowedDashboards.includes('announcer-dashboard')) {
        return NextResponse.json(
    ...
    if (!user || (user.role !== 'announcer' && !user.isAdmin)) {
      return NextResponse.json(
        { success: false, error: 'Invalid credentials or insufficient permissions' },
        { status: 401 }
      );
    ...
    console.error('Announcer authentication error:', error);
    return NextResponse.json(
